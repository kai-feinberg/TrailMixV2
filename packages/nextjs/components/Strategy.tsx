...
const Strategy = ({ contractAddress, userAddress }: { contractAddress: string, userAddress: string}) => {
    ...
    const { data: latestPrice, isLoading: isLoadingLatestPRice, error: errorLatestPrice } = useContractRead({
        address: contractAddress,
        abi: strategyABI,
        functionName: "getLatestPrice",
    });

    const { data: tslThreshold, isLoading: isLoadingTSLThreshold, error: errorTSLThreshold } = useContractRead({
        address: contractAddress,
        abi: strategyABI,
        functionName: "getTSLThreshold",
    });

    const { data: isTSLActive, isLoading: isLoadingIsTSLActive, error: errorIsTSLActive } = useContractRead({
        address: contractAddress,
        abi: strategyABI,
        functionName: "isTSLActive",
    });

    const { data: uniswapRouterAddress, isLoading: isLoadingUniswapRouterAddress, error: errorUniswapRouterAddress } = useContractRead({
        address: contractAddress,
        abi: strategyABI,
        functionName: "getUniswapRouterAddress",
    });

    const { data: trailAmount, isLoading: isLoadingTrailAmount, error: errorTrailAmount } = useContractRead({
        address: contractAddress,
        abi: strategyABI,
        functionName: "getTrailAmount",
    });

    const { data: manager, isLoading: isLoadingManager, error: errorManager } = useContractRead({
        address: contractAddress,
        abi: strategyABI,
        functionName: "getManager",
    });

    const { data: creator, isLoading: isLoadingCreator, error: errorCreator } = useContractRead({
        address: contractAddress,
        abi: strategyABI,
        functionName: "getCreator",
    });

    const { data: granularity, isLoading: isLoadingGranularity, error: errorGranularity } = useContractRead({
        address: contractAddress,
        abi: strategyABI,
        functionName: "getGranularity",
    });

    const { data: uniswapPool, isLoading: isLoadingUniswapPool, error: errorUniswapPool } = useContractRead({
        address: contractAddress,
        abi: strategyABI,
    ...
    const handleDeposit = async () => {
        if (!readyToDeposit) {
            ...
            if (approve) { // Add null check for approve function
                await approve();
            }
            setReadyToDeposit(true);
        }

        if (readyToDeposit && deposit) { // Add null check for deposit function
